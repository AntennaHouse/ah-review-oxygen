        ...
        <xsl:choose>
            <xsl:when test="$prmInsertElement[. is $elem] => exists()">
                ...
                <xsl:variable name="insertFoProp" as="attribute()?">
                    <xsl:variable name="foProp" as="attribute()" select="ahf:getChangeTrackingFoPropertyInsert($elem)"/>
                    <xsl:copy-of select="ahf:addColorToFoProp($foProp,ahf:getInsertFgColorSpecFromPi($insertStartSurroundPi)) => ahf:addInsertDecorationToFoProp() => ahf:filterEmptyAttr()"/>
                </xsl:variable>
                <xsl:copy>
